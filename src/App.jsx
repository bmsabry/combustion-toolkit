...
/* ══════════════════ PANELS ══════════════════ */
function AFTPanel({fuel,ox,phi,T0,P,Tfuel,combMode,setCombMode}){
  ...
  const bk=useBackendCalc("aft",{fuel:nonzero(fuel),oxidizer:nonzero(ox),phi,T0,P:atmToBar(P),mode:"adiabatic",heat_loss_fraction:0,T_fuel_K:Tfuel,T_air_K:Tair},accurate);
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
    ...
    <div style={S.card}><div style={S.cardT}>Fuel & Combustion Properties {combMode==="equilibrium"&&!accurate&&<span style={{color:C.accent,fontWeight:400}}> — Equilibrium Mode</span>}{statusBadge}</div>
      <div style={{...S.row,gap:8}}>
        ...
        <M l="Modified Wobbe Index (MWI)" v={uv(units,"energy_vol",props.LHV_vol/Math.sqrt(Math.max(props.SG,1e-9)*Tfuel/288.89)).toFixed(units==="SI"?2:1)} u={uu(units,"energy_vol")} c={C.violet} tip={`MWI = LHV_vol / √(SG × T_fuel / T_ref),  T_ref = 520°R = 288.89 K. GE/Siemens heavy-duty GT fuel-interchangeability index — T/T_ref is dimensionless, so MWI carries the same units as LHV_vol. At T_fuel = T_ref, MWI = LHV-based Wobbe. Evaluated at T_fuel = ${uv(units,"T",Tfuel).toFixed(0)} ${uu(units,"T")}.`}/>
...
function CombustorPanel({fuel,ox,phi,T0,P,tau,setTau,Lpfr,setL,Vpfr,setV,Tfuel,setTfuel,psrSeed,setPsrSeed,eqConstraint,setEqConstraint,integration,setIntegration,heatLossFrac,setHeatLossFrac,mechanism,setMechanism}){
  const units=useContext(UnitCtx);
  const {accurate}=useContext(AccurateCtx);
...
function ExhaustPanel({fuel,ox,T0,P,Tfuel,measO2,setMeasO2,measCO2,setMeasCO2,combMode,setCombMode}){
  const units=useContext(UnitCtx);
  const {accurate}=useContext(AccurateCtx);
  const Tair=T0;
  ...
  const bkO2=useBackendCalc("exhaust",{fuel:nonzero(fuel),oxidizer:nonzero(ox),T0,P:atmToBar(P),measured_O2_pct_dry:measO2,combustion_mode:combMode,T_fuel_K:Tfuel,T_air_K:Tair},accurate);
  const bkCO2=useBackendCalc("exhaust",{fuel:nonzero(fuel),oxidizer:nonzero(ox),T0,P:atmToBar(P),measured_CO2_pct_dry:measCO2,combustion_mode:combMode,T_fuel_K:Tfuel,T_air_K:Tair},accurate);
